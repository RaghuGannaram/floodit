...
function hexToRgb(hex) {
	...
	const rPart = hex.substring(0, 2);
	const gPart = hex.substring(2, 4);
	const bPart = hex.substring(4, 6);
...
function renderTable() {
	...
	for (let i = 1; i <= gameProperties.rowCount; i++) {
		...
		for (let j = 1; j <= gameProperties.columnCount; j++) {
			...
			td.onclick = (event) => {
				if (td.style.backgroundColor !== hexToRgb(gameProperties.obstacleCellColor)) {
...
function clearTable() {
	for (let i = 1; i <= gameProperties.rowCount; i++) {
		for (let j = 1; j <= gameProperties.columnCount; j++) {
			if (
				document.getElementById(`${i}-${j}`).getAttribute("data-state") === "flooded" ||
				document.getElementById(`${i}-${j}`).getAttribute("data-state") === "source-flooded"
			) {
